...
## The generated file is committed, so it conflicts

`TODO.md` is generated *and committed*, so two agents closing two different items both rewrite
it, and git — which has no idea the file is derived — conflicts on the merge. Any repository
with two agents in it meets this almost at once: one project hit it four times in a single
afternoon, every time as the only conflicted file and every time fixed by `artisan todo:index`
and carrying on. There are two halves to the answer, and the first one is a line in the
...
### Union covers git, not GitHub

The attribute is applied by the merge that runs in *your* checkout — `merge`, `rebase`, `pull`,
`merge-tree`. GitHub's mergeability check and its merge button do not apply it. Measured on a
live pull request whose only shared file was the index: `git merge-tree --merge-base=origin/main
origin/main <branch>` exited 0 with a clean tree while the API reported `mergeable: false,
mergeable_state: dirty` for that same pair of commits. Locally clean, unmergeable through the
button.

Where conflicts are occasional that is survivable — pull, regenerate, push, and the attribute
means the pull itself never stops to ask. Where **every** branch closes an item it is not. One
project works to a rule of one pull request per item, which makes the top of `## Done` the block
that every branch inserts into, so any two open pull requests collide by construction rather
than by luck: two unrelated merges one afternoon turned four open pull requests CONFLICTING at
once, none of which had touched anything the others had.

### So let only the default branch write it

The second half, for a repository shaped like that. Set `auto_index` false, and `todo:new` and
`todo:done` stop rewriting the index:

```php
// config/todo-items.php — published with `vendor:publish --tag=todo-items-config`
'auto_index' => false,
```

They still write the item file, which is the truth, and `todo:list` still reads the item files,
so nothing you *ask* goes stale — only the rendered file does. `todo:index` is untouched by the
flag, because it is the command named for doing exactly this, and it is what the default branch
now runs after each merge:

```yaml
# .github/workflows/todo-index.yml, on: push: branches: [main]
- run: php artisan todo:index
- run: |
    git config user.name "github-actions[bot]"
    git config user.email "41898282+github-actions[bot]@users.noreply.github.com"
    git add todo TODO.md
    git diff --cached --quiet || { git commit -m "Regenerate TODO.md"; git push; }
```

No branch then writes the file, so two branches have nothing to conflict over — the conflict is
not merged better, it stops being produced. Keep the `merge=union` line anyway: it costs nothing
and it still covers the index on the default branch, where the regenerate commit and a
fast-moving merge queue can still meet.

Three things to know before adopting it:

- **`git add todo`, not just the index.** Regenerating *adopts* hand-written bullets into real
  item files (below). A job that commits only `TODO.md` writes the adopted row and throws away
  the file behind it, and the next regenerate then drops the row — the one path here that loses
  an item rather than merely rendering it wrong.
- **Don't commit a branch's regenerate.** Running `todo:index` on a branch is fine and often
  what you want, to read a current index. Committing the result puts the file back in the diff
  and the conflict back with it. `git checkout -- TODO.md` before you commit.
- **A protected default branch will refuse the push.** The usual repository that needs this is
  also the repository whose `main` only moves by pull request, and `GITHUB_TOKEN` is not exempt.
  Allow the app to bypass the rule, push with an app or PAT token that can, or accept an index
  that is regenerated by hand. This is worth settling before the flag goes in, not after.

Set the flag in the published config rather than reading it from `.env`. It has to hold for
every clone and every agent at once, and a `.env` that one machine is missing puts exactly that
machine back to writing the file nobody else writes.
...
`storage/framework/`, the commands present only in `local` and `testing`, and `auto_index` true
so `todo:new` and `todo:done` keep the index in step ([above](#so-let-only-the-default-branch-write-it)
for when to turn that off). Publish to change any of it:
